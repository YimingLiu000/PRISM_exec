...
# 用途：
# 1. 在服务器上并行下载 PrackenDB（单个超大 tar.gz 文件）
# 2. 支持断点续传
...
# 可调参数：
#   PROJECT_ROOT      项目目录
#   OUT_DIR           输出目录，默认 ${PROJECT_ROOT}/02ref/kraken2_prackendb
#   URL               下载链接
...
URL="${URL:-https://genome-idx.s3.amazonaws.com/kraken/k2_NCBI_reference_20251007.tar.gz}"
OUT_DIR="${OUT_DIR:-${PROJECT_ROOT}/02ref/kraken2_prackendb}"
...
EXPECTED_SIZE=""
EXPECTED_SIZE="$(curl -fsSI "${URL}" | awk 'BEGIN{IGNORECASE=1} /^Content-Length:/ {gsub("\r","",$2); print $2; exit}' || true)"
